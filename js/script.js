    ...
    try {
      ...
      const res = await fetch('https://api.web3forms.com/submit', {
        ...
        body: JSON.stringify({
          ...
          Modelo: state.modelo || '—',
          Ano: state.ano || '—',
          'Cidade (informada)': state.cidade || '—',
          'Valor do veículo': formatCurrencyBR(state.valor),
          'Data e hora do envio': dataHoraEnvio(),
          ...(geoData.bairro   ? { 'Bairro (localização)':  geoData.bairro } : {}),
          ...(geoData.cidade   ? { 'Cidade (localização)':  `${geoData.cidade}, ${geoData.estado}` } : {}),
          ...(geoData.cep      ? { 'CEP (localização)':     geoData.cep } : {}),
